...
conditional orders are triggered. The modern API expects string values instead
of the historical numeric codes:

| Direction | API value   | Description |
|-----------|-------------|-------------|
| Rising    | `ascending` | Trigger when the reference price moves upwards to the trigger price. |
| Falling   | `descending`| Trigger when the reference price moves downwards to the trigger price. |
